...
public class AnimalDao extends Dao<Integer, AnimalVo> {
	...
	public void insert(AnimalVo v) throws Exception {
		...
		try {
			con = getConnection();	
			ps = con.prepareStatement(Sql.insertAnimal);
			ps.setInt(1, v.getId());
	...
	public void update(AnimalVo v) throws Exception {
		...
		try {
			con = getConnection();
			ps = con.prepareStatement(Sql.updateAnimal);
	...
	public void delete(Integer k) throws Exception {
		...
		try {
			con = getConnection();
			ps = con.prepareStatement(Sql.deleteAnimal);
			ps.setInt(1, k);
	...
	public AnimalVo select(Integer k) throws Exception {
		...
		try {
			con = getConnection();
			ps = con.prepareStatement(Sql.selectAnimal);
			ps.setInt(1, k);
			rs = ps.executeQuery();
			rs.next();
			int id = rs.getInt("id");
	...
	public List<AnimalVo> select() throws Exception {
		...
		try {
			con = getConnection();
			ps = con.prepareStatement(Sql.selectAllAnimal);
			rs = ps.executeQuery();			
			while(rs.next()) {
				int id = rs.getInt("id");
				String species = rs.getString("species");
